...
describe('API Service - Autenticación', () => {
  ...
  describe('Login', () => {
    it('debería iniciar sesión correctamente', async () => {
      ...
      expect(callArgs[0]).toBe('https://mivideoteca-backend-anabelengonzalezisasi.onrender.com/api/auth/login');
  ...
  describe('Registro', () => {
    it('debería registrar usuario correctamente', async () => {
      ...
      const callArgs = (globalThis.fetch as any).mock.calls[0];
      expect(callArgs[0]).toBe('https://mivideoteca-backend-anabelengonzalezisasi.onrender.com/api/auth/register');
      expect(callArgs[1].method).toBe('POST');
      expect(callArgs[1].body).toBe(JSON.stringify({ email, password }));
    });
  ...
  describe('Peticiones con autenticación', () => {
    it('debería incluir token en el header Authorization', async () => {
      ...
      const callArgs = (globalThis.fetch as any).mock.calls[0];
      expect(callArgs[0]).toBe('https://mivideoteca-backend-anabelengonzalezisasi.onrender.com/api/movies');
...
describe('API Service - Películas', () => {
  ...
  it('debería marcar/desmarcar una película como favorita', async () => {
    ...
    const callArgs = (globalThis.fetch as any).mock.calls[0];
    expect(callArgs[0]).toBe(`https://mivideoteca-backend-anabelengonzalezisasi.onrender.com/api/movies/${movieId}/toggle-favorite`);
